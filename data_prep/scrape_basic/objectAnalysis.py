...
def rawcoinscount(commentobjectarray):
    ...
    for comment in commentobjectarray:
        if comment.coinsmentioned:
            for coin in comment.coinsmentioned:
                if coin not in rawcoindictups and coin not in other_stop_words:
                    rawcoindictups[coin] = {}
                    rawcoindictsent[coin] = {}
                    rawcoindictups[coin][comment.createdutc] = comment.ups
                    rawcoindictsent[coin][comment.createdutc] = comment.sentiment
...
def returntotalcountpercoin():
    ...
    timearray = timesort.timearray_pastxintervals(3600,1500)  # time array for coinsovertime (interval in seconds, length)
